...
public class TireController {
  ...
  @ApiResponses({@ApiResponse(responseCode = "200", description = "Zwrócono listę opon"),
  ...
  public ResponseEntity<TireResponse> getTireById(@PathVariable Long id) {
    TireResponse tire = tireService.getTireById(id).orElseThrow(
        () -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Nie znaleziono opony"));
  ...
  @ApiResponses({@ApiResponse(responseCode = "200", description = "Dodano opone"),
  ...
  @ApiResponses({@ApiResponse(responseCode = "200", description = "Edytowano opone"),
      @ApiResponse(responseCode = "403", description = "Brak autoryzacji lub uprawnień")})
  @PatchMapping("/api/admin/tire/{id}")
  @PreAuthorize("hasRole('ADMIN')")
  public ResponseEntity<?> updateTire(@PathVariable Long id,
  ...
  @ApiResponses({@ApiResponse(responseCode = "200", description = "Usunięto opone"),
      @ApiResponse(responseCode = "403", description = "Brak autoryzacji lub uprawnień")})
