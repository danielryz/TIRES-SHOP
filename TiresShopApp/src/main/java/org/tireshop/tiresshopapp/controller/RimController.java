...
public class RimController {
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "Zwrócono listę felg"),
    ...
    public ResponseEntity<RimResponse> getRimById(@PathVariable Long id) {
        RimResponse rim = rimService.getRimById(id)
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Nie znaleziono felgi"));
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "Dodano felge"),
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "Edytowano felge"),
            @ApiResponse(responseCode = "403", description = "Brak autoryzacji lub uprawnień")
    })
