...
public class ProductController {
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "Zwrócono dane produktu"),
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "201", description = "Produkt został utworzony"),
            @ApiResponse(responseCode = "403", description = "Brak autoryzacji lub uprawnień")
    })
    @PreAuthorize("hasRole('ADMIN')")
    @PostMapping("/api/admin/products")
    public ResponseEntity<ProductResponse> createNewProduct(@RequestBody CreateProductRequest request) {
        ProductResponse response = productService.createProduct(request);
        return ResponseEntity.status(HttpStatus.CREATED).body(response);
    }

    @Operation(summary = "Częściowa edycja produktu", description = "Aktualizacja wybranych pól (tylko admin)")
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "Zaktualizowano produkt"),
            @ApiResponse(responseCode = "404", description = "Nie znaleziono produktu")
    })
    @PreAuthorize("hasRole('ADMIN')")
