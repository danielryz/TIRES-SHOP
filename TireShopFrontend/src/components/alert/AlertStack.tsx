...
const AlertStack = ({ alerts, onRemove }: Props) => {
    ...
    useEffect(() => {
        const timers = alerts.map((alert) => {
            const fadeTimer = setTimeout(() => setFadeOutIds((ids) => [...ids, alert.id]), 3700);
            const removeTimer = setTimeout(() => onRemove(alert.id), 4000);
        ...
        <div className="alert-stack">
            {alerts.map((alert) => (
                ...
                >
                    ...
                    <button className="alert-close" onClick={() => onRemove(alert.id)}>×</button>
                </div>
            ))}
        </div>
    );
