            ...
            {items.length === 0 ? (
                ...
                <>
                            ...
                            <div key={item.id} className="cart-item">
                                ...
                                <div className="cart-info">
                                    ...
                                    <div className="cart-quantity">
                                        <button onClick={() => handleUpdate(item.id, item.quantity - 1)} disabled={item.quantity <= 1}>-</button>
                                        <span>{item.quantity}</span>
                                        <button onClick={() => handleUpdate(item.id, item.quantity + 1)}>+</button>
                                    </div>
                                    <p><strong>Suma:</strong> {item.totalPrice.toFixed(2)} zł</p>
                                    <button className="remove-btn" onClick={() => handleDelete(item.id)}>Usuń</button>
                                </div>
                            </div>
                        ))}
                    </div>
                    ...
                    <div className="cart-summary">
                        <p><strong>Łącznie:</strong> {total.toFixed(2)} zł</p>
                        <button className="clear-btn" onClick={handleClear}>Wyczyść koszyk</button>
                        <button
